...
\textbf{Want to schedule an online appointment to chat?}
Use our \href{https://outlook.office365.com/owa/calendar/UCCenterofExcellenceonUASSafety@merced.onmicrosoft.com/bookings/}{Booking link}. Find a time that works for you and the system will schedule a MS Teams meeting for us.

\begin{notebox}
Projects supported with Federal Funds may not use drones manufactured in China. Please be aware that recent federal regulations may restrict the types of drones that you use and may impact the availability of certain drones.

\textbf{Due to recent regulatory actions, We recommend buying only drones fully manufactured within the US}

\end{notebox}

\subsection*{UC Drone Policy}\label{uc-drone-policy}
\addcontentsline{toc}{subsection}{UC Drone Policy}
...
Head to the FAA DroneZone (\url{https://faadronezone.faa.gov}) \ref{fig:reg-page} and select \textbf{``I fly under Part 107 or as a Public Aircraft''}
...
\tightlist
\item
  In the center of your dashboard is an Inventory box - click on \textbf{`Manage Device Inventory'}
\item
  The next page is a little strange - the top headline says \textbf{`Your Shopping Cart'} and if nothing is in your shopping cart, there'll be a big red notice that your shopping cart is empty. If you have drones already registered, they'll be listed at the bottom under \textbf{`Your Registered Inventory'}. Take a moment to understand the layout of this page.
\item
  Click on the \textbf{`Add Device'} button in the upper-right corner to get started.
\item
  On the first dialog, it'll ask whether the drone has \textbf{Remote ID}. (Figure \ref{fig:add-UAS})
\end{enumerate}

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{images/Add_new_UAS_1} 
...
\setcounter{enumi}{4}
\item
  If you select `Yes' to RemoteID, it will ask for the serial number (Figure \ref{fig:add-UAS-RID}). Click on \textbf{Validate} to continue - this will pull up the serial number's make and model.

  \begin{itemize}
  \tightlist
  \item
    If your drone has RemoteID built-in (known as Standard RemoteID), enter your drone's serial number
  \item
    If you're using a Broadcast Remote ID module (add-on), enter the Broadcast Remote ID module's serial number
  \end{itemize}
\end{enumerate}

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{images/Add_new_UAS_2} 

}

\caption{Validate Remote ID}\label{fig:add-UAS-RID}
\end{figure}
...
\tightlist
\item
  If you selected `No' to RemoteID, the dialog will ask what \textbf{type of UAS} (`Homebuilt' or `Traditional') and ask for the UAS \textbf{Manufacturer} and \textbf{Model} (Figure \ref{fig:add-UAS-RID-None}).
\end{enumerate}

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{images/Add_new_UAS_0} 

}

\caption{No Remote ID}\label{fig:add-UAS-RID-None}
\end{figure}
...
\tightlist
\item
  Whether you selected `Yes' or `No' to RemoteID, the dialog will ask for a \textbf{Nickname} (Figure \ref{fig:add-UAS-Valid}) - this will be your identifier for the drone while within the FAA DroneZone system, so make it something identifiable. You don't use this nickname anywhere else, so you don't need to be creative if you don't want to be.
\end{enumerate}

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{images/Add_new_UAS_3} 

}

\caption{Serial Number Validated}\label{fig:add-UAS-Valid}
\end{figure}
...
\tightlist
\item
  Click on \textbf{`Add Device'} to move this to your shopping cart.
\item
  Back at the Inventory page, `Your Shopping Cart' will now show this drone's information and a new buttom for \textbf{`Checkout'} will appear. Click the button and the system will guide you through paying for the \$5 registration fee.
\end{enumerate}
...
\begin{notebox}
We recommend that you \textbf{upload} the FAA registration certificate to \textbf{UC Drones} as an additional means of keeping the certificate available.
...
\href{https://ucdrones.github.io/library/reg_tag_generator.html}{UC Drones Registration Tag Generator} - We also have a UC Drones registration tag generator - enter the drone's nickname, your campus and the registration number to generate a simple tag you can print and attach to the drone.
...
The full list of FAA-certified drones can be found here: \url{https://uasdoc.faa.gov/listDocs}, filter by OOP - There are very few entries, but a couple of decent options including:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  AgEagle eBee X (Category 3)
\item
  Wingtra Ray (Category 3)
\item
  ASTM F3322 Compliant Parachute Systems by AVSS for Mavic 3, Skydio X10, Matrice 4 platforms
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/FAA-VFR.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{images/FAA-Notam.png}}
...
\href{http://ehs.ucop.edu/drones}{\pandocbounded{\includegraphics[keepaspectratio]{images/UCDrones_logo.jpg}}}
...
After each flight request, please make sure to file a post-flight report. You can find a list of all of your flights awaiting a post-flight report here: \href{https://ehs.ucop.edu/drones/flight?requestApproved=true}{Report on Approved Flights} (requires log-in)

\begin{notebox}
Make sure to click on \textbf{Add Flight} whenever you utilize a Project. Otherwise, your flights won't be listed for reporting.
...
\subsection{Filing a Post-Flight Report}\label{filing-a-post-flight-report}

Find the flight that you wish to report on and enter it by clicking the the flight name. You can find a list of all of your flights awaiting a post-flight report here: \href{https://ehs.ucop.edu/drones/flight?requestApproved=true}{Report on Approved Flights} (requires log-in)

At the bottom of your flight request, click on the \textbf{Create Report} button (Figure \ref{fig:UCDrones-post-flight-1}).

\begin{figure}

{\centering \includegraphics[width=0.85\linewidth]{images/UCDrones_flight_report_button} 

}

\caption{UC Drones Create Report Button}\label{fig:UCDrones-post-flight-1}
\end{figure}
...
\tightlist
\item
  \textbf{Duration per Flight} - On each row, list the minutes of each flight. For example, if you had three flights, you would list them as in Figure \ref{fig:UCDrones-post-flight-2}.
\end{enumerate}

\begin{figure}

{\centering \includegraphics[width=0.65\linewidth]{images/UCDrones_flight_report_flight} 

}

\caption{UC Drones Add Additional Flights}\label{fig:UCDrones-post-flight-2}
\end{figure}
...
\setcounter{enumi}{1}
\item
  \textbf{Takeoff and Landing Damages} - Select whether the drone encountered damage while taking off or landing. If so, select the scale of the damage: minor damages, major, or total loss.
\item
  \textbf{Equipment Malfunctions} - If the drone suffered an equipment malfunction, select which component failed.
\item
  \textbf{Lost Link Events} - If the drone lost communication with the control station, pilot or other safety crew at any point (even temporarily), select the type of loss.
\item
  \textbf{Accidents/Mishaps} - If an accident or incident occurred, select one of the options (property damage, injury to non-participant, adverse wildlife, etc).
\end{enumerate}

Once you have entered in the post-flight data, click on the \textbf{Save} button at the bottom (Figure \ref{fig:UCDrones-post-flight-3}).

\begin{figure}

{\centering \includegraphics[width=0.85\linewidth]{images/UCDrones_flight_report_save} 

}

\caption{UC Drones Save Report}\label{fig:UCDrones-post-flight-3}
\end{figure}
...
{\centering \includegraphics[width=0.85\linewidth]{images/UCDrones_flight_report_review} 

}

\caption{UC Drones Review}\label{fig:UCDrones-post-flight-4}
\end{figure}

At the very bottom of the page, select the \textbf{Edit Report} to go back and make any changes, or \textbf{Submit Report} to complete the reporting process (Figure \ref{fig:UCDrones-post-flight-5}).

\begin{figure}

{\centering \includegraphics[width=0.85\linewidth]{images/UCDrones_flight_report_submit} 

}

\caption{UC Drones Submit Report}\label{fig:UCDrones-post-flight-5}
\end{figure}

\subsection{Non-Punitive Reporting Statement}\label{non-punitive-reporting-statement}
...
\caption{The Mavic 3 is a great DJI model, but is it the best drone for you?}\label{fig:drone3}
\end{figure}
