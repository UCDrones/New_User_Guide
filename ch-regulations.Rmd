...
Head to the FAA DroneZone (https://faadronezone.faa.gov) \@ref(fig:reg-page) and select **"I fly under Part 107 or as a Public Aircraft"** 
...
1. In the center of your dashboard is an Inventory box - click on **'Manage Device Inventory'**
2. The next page is a little strange - the top headline says **'Your Shopping Cart'** and if nothing is in your shopping cart, there'll be a big red notice that your shopping cart is empty. If you have drones already registered, they'll be listed at the bottom under **'Your Registered Inventory'**. Take a moment to understand the layout of this page.
3. Click on the **'Add Device'** button in the upper-right corner to get started.
4. On the first dialog, it'll ask whether the drone has **Remote ID**. (Figure \@ref(fig:add-UAS))

```{r add-UAS, fig.cap='Add new UAS Dialog Box', out.width='80%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics('images/Add_new_UAS_1.png')
```

5. If you select 'Yes' to RemoteID, it will ask for the serial number (Figure \@ref(fig:add-UAS-RID)). Click on **Validate** to continue - this will pull up the serial number's make and model.
...
```{r add-UAS-RID, fig.cap='Validate Remote ID', out.width='80%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics('images/Add_new_UAS_2.png')
```

6. If you selected 'No' to RemoteID, the dialog will ask what **type of UAS** ('Homebuilt' or 'Traditional') and ask for the UAS **Manufacturer** and **Model** (Figure \@ref(fig:add-UAS-RID-None)). 

```{r add-UAS-RID-None, fig.cap='No Remote ID', out.width='80%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics('images/Add_new_UAS_0.png')
```

7. Whether you selected 'Yes' or 'No' to RemoteID, the dialog will ask for a **Nickname** (Figure \@ref(fig:add-UAS-Valid)) - this will be your identifier for the drone while within the FAA DroneZone system, so make it something identifiable. You don't use this nickname anywhere else, so you don't need to be creative if you don't want to be.

```{r add-UAS-Valid, fig.cap='Serial Number Validated', out.width='80%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics('images/Add_new_UAS_3.png')
```

8. Click on **'Add Device'** to move this to your shopping cart.
9. Back at the Inventory page, 'Your Shopping Cart' will now show this drone's information and a new buttom for **'Checkout'** will appear.  Click the button and the system will guide you through paying for the $5 registration fee.
...
:::: {.notebox data-latex=""}
We recommend that you **upload** the FAA registration certificate to **UC Drones** as an additional means of keeping the certificate available.
::::
...
[UC Drones Registration Tag Generator](https://ucdrones.github.io/library/reg_tag_generator.html) - We also have a UC Drones registration tag generator - enter the drone's nickname, your campus and the registration number to generate a simple tag you can print and attach to the drone.
...
The full list of FAA-certified drones can be found here: https://uasdoc.faa.gov/listDocs, filter by OOP - There are very few entries, but a couple of decent options including:

1. AgEagle eBee X (Category 3)
2. Wingtra Ray (Category 3)
3. ASTM F3322 Compliant Parachute Systems by AVSS for Mavic 3, Skydio X10, Matrice 4 platforms
